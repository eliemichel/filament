...
void RenderPass::setupColorCommand(Command& cmdDraw,
    ...
    if (!isBlendingCommand && Variant::isSSRVariant(variant)) {
        // for SSR pass, the blending mode of opaques (including MASKED) must be off
        // see Material.cpp.
        cmdDraw.primitive.rasterState.blendFunctionSrcAlpha = BlendFunction::ONE;
        cmdDraw.primitive.rasterState.blendFunctionDstAlpha = BlendFunction::ZERO;
    }
